...
path_inputs = inputs.map { |input| input.split(',') }
wire_paths = []

path_inputs.each do |trace|
  wire_path = []
  point = [0, 0]
...
intersections = wire_paths[0] & wire_paths[1]

closest_point = intersections.min_by { |point| point[0].abs + point[1].abs }

puts "Part 1: #{closest_point.map(&:abs).sum}"

fewest_steps_point = intersections.min_by { |point| wire_paths[0].index(point) + wire_paths[1].index(point) }

puts "Part 2: #{wire_paths[0].index(fewest_steps_point) + wire_paths[1].index(fewest_steps_point) + 2}"
